 ...
 * any prerendered SvelteKit site: copy this file + $lib/seo/robots + the
 * /sitemap.xml/+server.ts endpoint (and keep the endpoint in
 * `prerender.entries`; nothing links to it).
 *
 * The URL list is every `+page.svelte` under src/routes, minus
 *   - the ROBOTS_DISALLOW prefixes ($lib/seo/robots — shared with the
 *     robots.txt route, so a disallowed path can't be advertised here);
